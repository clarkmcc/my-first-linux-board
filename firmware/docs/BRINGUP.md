...
Run these commands from `boot-console/firmware`:
...
UART0 PE0/PE1 selection, SDC0 settings, PC0 active-low LED, native USB peripheral
mode, and disabled SPI0. It also checks that boot arguments, partition numbering,
and getty startup agree. A source-only review or successful DTS preprocessing is
not a substitute for this command.
...
- `/dev/mmcblk0p2` is the ext4 root partition and the kernel command line mounts
  it read-only.
- `CONFIG_USB_G_SERIAL=y` is built in. The development USB identity
  `0525:a4a7` is expected until a production VID/PID is assigned.
...
successful hardware boot; `CONFIG_SPI=n` and `CONFIG_NET=n` remain required now.
...
5. The green status LED begins its heartbeat pattern.
...
the host enumerates Linux CDC ACM device `0525:a4a7`, `/dev/ttyGS0` exists on the
board, and a 115200 terminal reaches a login prompt. Disconnect and reconnect the
host several times. The UART console must remain usable, and the USB getty must
not enter a rapid restart loop while disconnected.
...
USB PHY/controller, gadget driver, cable, or host enumeration. Record the full
UART log and measured rails before changing the image or board.
